...
function unfold(spring) {
  const rules = spring.rules;
  const pattern = spring.pattern;

  spring.pattern = Array(5).fill(pattern).join("?");
  spring.rules = Array(5).fill(rules).join(",");

  spring.rules = spring.rules.split(",").map(Number);
