...
const count = game => {
  c = game.winning.reduce((acc, cur) => game.mynums.includes(cur) 
...
async function myFileReader() {
  ...
  for await (const line of file.readLines()) {
    ...
    let win = Array.from(numbers[0].trim().matchAll(/(\d+)/g), match => match[1])
    let my  = Array.from(numbers[1].trim().matchAll(/(\d+)/g), match => match[1])
  ...
  stack = processCards(stack);

  console.log("Part 2: " + totalCards(stack));
