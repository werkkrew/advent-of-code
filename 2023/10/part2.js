...
function navigate(map) {
  ...
  while(!mapped) {
    ...
    if ( JSON.stringify(position) == JSON.stringify(start) ) {
      mapped = true;
      steps++
      console.log("Steps around loop is " + steps + " steps to midpoint is " + ( steps/2 ))
    } else {
      steps++
...
function expand(map) {
  ...
  for ( let y = 0; y < map.length; y++ ) {
    ...
    for ( let x = 0; x < row.length; x++ ) {
      pipe = map[y][x]
      exprow.push(map[y][x])
      if (pipe == "." || pipe == "7" || pipe == "J" || pipe == "|") exprow.push(".")
      if (pipe == "F" || pipe == "-" || pipe == "L" || pipe == "S") exprow.push("-")
    }
    ...
    for ( let x = 0; x < exprow.length; x++ ) {
      pipe = exprow[x]
      if (pipe == "." || pipe == "-" || pipe == "J" || pipe == "L") newrow.push(".")
      if (pipe == "7" || pipe == "F" || pipe == "|" || pipe == "S") newrow.push("|")
    }
      ...
      for ( let x = 0; x < row.length; x++ ) {
        ...
        for ( let i = 0; i < directions.length; i++ ) {
          ...
          while (!next) {
            ...
            if ( coords[0] < 0 || coords[0] >= maxY || coords[1] < 0 || coords[1] >= maxX) {
              next = true
            } else {
